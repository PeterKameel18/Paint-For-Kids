		...
		if (Fig->GetGfxInfo().isFilled)
		{
			...
			else if (Fig->GetGfxInfo().FillClr == YELLOW)
			{
				if (Fig->getType() == "Rectangle")
				{
					...
					pOut->PrintMessage("Pick up all the yellow rectangles!");

				}
				else if (Fig->getType() == "Triangle")
				{
					picked_comb_no = combinations[6];
					pOut->PrintMessage("Pick up all the yellow triangles!");

				}
				else if (Fig->getType() == "Square")
				{
					picked_comb_no = combinations[7];
					pOut->PrintMessage("Pick up all the yellow squares!");

				}
				else if (Fig->getType() == "Hexagon") {
					picked_comb_no = combinations[8];
					pOut->PrintMessage("Pick up all the yellow Hexagons!");
				}
				else
				{
					picked_comb_no = combinations[9];
					pOut->PrintMessage("Pick up all the yellow Circles!");
				}
		...
		while (picked_comb_no > 0)
		{
			...
			if (P.y > UI.ToolBarHeight || P.x > (UI.MenuItemWidth * PLAY_ITM_COUNT))
			{
				...
				if (clickedFig != NULL)
				{
					...
					if (clickedFig->getType()=="Hexagon" && Fig->getType() == "Hexagon" && (clickedFig->GetGfxInfo().FillClr == Fig->GetGfxInfo().FillClr))
					{
						PrntScore(1);
						clickedFig->Hide();
